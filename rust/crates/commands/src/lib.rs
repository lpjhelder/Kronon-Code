...
const SLASH_COMMAND_SPECS: &[SlashCommandSpec] = &[
    ...
    SlashCommandSpec {
        ...
        summary: "Manage Kronon Code plugins",
        argument_hint: Some(
...
fn default_skill_install_root() -> std::io::Result<PathBuf> {
    ...
    Err(std::io::Error::new(
        std::io::ErrorKind::NotFound,
        "unable to resolve a skills install root; set KRONON_CONFIG_HOME or HOME",
    ))
}
...
fn render_agents_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       kronon agents".to_string(),
        "  Sources          .kronon/agents, ~/.kronon/agents, $KRONON_CONFIG_HOME/agents".to_string(),
    ];
...
fn render_agents_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            ...
            "direct_cli": "kronon agents [list|help]",
            "sources": [".kronon/agents", "~/.kronon/agents", "$KRONON_CONFIG_HOME/agents"],
        },
...
fn render_skills_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            ...
            "aliases": ["/skill"],
            "direct_cli": "kronon skills [list|install <path>|help|<skill> [args]]",
            "invoke": "/skills help overview -> $help overview",
            "install_root": "$KRONON_CONFIG_HOME/skills or ~/.kronon/skills",
            "sources": [
                ".kronon/skills",
                ".omc/skills",
                ".agents/skills",
...
fn render_mcp_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       kronon mcp [list|show <server>|help]".to_string(),
        "  Sources          .kronon/settings.json, .kronon/settings.local.json".to_string(),
    ];
    if let Some(args) = unexpected {
        lines.push(format!("  Unexpected       {args}"));
    }
...
fn render_mcp_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            ...
            "direct_cli": "kronon mcp [list|show <server>|help]",
            "sources": [".kronon/settings.json", ".kronon/settings.local.json"],
        },
        "unexpected": unexpected,
    })
}
...
mod tests {
    ...
    fn renders_per_command_help_detail() {
        ...
        assert!(help.contains("Summary          Manage Kronon Code plugins"));
        assert!(help.contains("Aliases          /plugins, /marketplace"));
    ...
    fn renders_agents_reports_as_json() {
        ...
        assert_eq!(help["usage"]["direct_cli"], "kronon agents [list|help]");
    ...
    fn resolves_project_skills_and_legacy_commands_from_shared_registry() {
        ...
        let legacy_commands = workspace.join(".kronon").join("commands");
    ...
    fn renders_skills_reports_as_json() {
        ...
        assert_eq!(
            ...
            "kronon skills [list|install <path>|help|<skill> [args]]"
        );
    ...
    fn agents_and_skills_usage_support_help_and_unexpected_args() {
        ...
        assert!(agents_help.contains("Direct CLI       kronon agents"));
        assert!(agents_help
            .contains("Sources          .kronon/agents, ~/.kronon/agents, $KRONON_CONFIG_HOME/agents"));
        ...
        assert!(skills_help.contains("Install root     $KRONON_CONFIG_HOME/skills or ~/.kronon/skills"));
    ...
    fn mcp_usage_supports_help_and_unexpected_args() {
        ...
        assert!(help.contains("Direct CLI       kronon mcp [list|show <server>|help]"));
    ...
    fn renders_mcp_reports_from_loaded_config() {
        ...
        fs::create_dir_all(workspace.join(".kronon")).expect("workspace config dir");
        fs::create_dir_all(&config_home).expect("config home");
        fs::write(
            workspace.join(".kronon").join("settings.json"),
            r#"{
              "mcpServers": {
                "alpha": {
                  "command": "uvx",
        ...
        fs::write(
            workspace.join(".kronon").join("settings.local.json"),
            r#"{
              "mcpServers": {
                "remote": {
                  "type": "ws",
    ...
    fn renders_mcp_reports_as_json() {
        ...
        assert_eq!(help["action"], "help");
        assert_eq!(help["usage"]["sources"][0], ".kronon/settings.json");
