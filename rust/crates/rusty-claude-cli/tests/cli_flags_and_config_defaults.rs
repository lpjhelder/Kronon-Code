...
fn status_command_applies_model_and_permission_mode_flags() {
    ...
    // when
    let output = Command::new(env!("CARGO_BIN_EXE_kronon"))
...
fn slash_command_names_match_known_commands_and_suggest_nearby_unknown_ones() {
    ...
    // when
    let help_output = Command::new(env!("CARGO_BIN_EXE_kronon"))
        .current_dir(&temp_dir)
        .arg("/help")
        .output()
        .expect("kronon should launch");
    let unknown_output = Command::new(env!("CARGO_BIN_EXE_kronon"))
        .current_dir(&temp_dir)
        .arg("/zstats")
        .output()
        .expect("kronon should launch");
...
fn config_command_loads_defaults_from_standard_config_locations() {
    ...
    fs::create_dir_all(temp_dir.join(".kronon")).expect("project config dir should exist");
    ...
    fs::write(temp_dir.join(".kronon.json"), r#"{"model":"sonnet"}"#)
        .expect("write project settings");
    fs::write(
        temp_dir.join(".kronon").join("settings.local.json"),
        r#"{"model":"opus"}"#,
    )
    ...
    assert!(stdout.contains(temp_dir.join(".kronon.json").to_str().expect("utf8 path")));
    assert!(stdout.contains(
        temp_dir
            .join(".kronon")
...
fn doctor_command_runs_as_a_local_shell_entrypoint() {
    ...
    let output = command_in(&temp_dir)
        ...
        .arg("doctor")
        .output()
        .expect("kronon doctor should launch");
...
fn local_subcommand_help_does_not_fall_through_to_runtime_or_provider_calls() {
    ...
    assert!(doctor_stdout.contains("Usage            kronon doctor"));
    ...
    assert!(status_stdout.contains("Usage            kronon status"));
