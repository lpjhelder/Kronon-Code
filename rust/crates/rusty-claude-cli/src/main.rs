...
fn main() {
    if let Err(error) = run() {
        ...
        if message.contains("`kronon --help`") {
            eprintln!("error: {message}");
        } else {
            eprintln!(
                "error: {message}

Run `kronon --help` for usage."
            );
        }
        std::process::exit(1);
    }
...
fn parse_args(args: &[String]) -> Result<CliAction, String> {
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "-p" => {
                // Kronon Code compat: -p "prompt" = one-shot prompt
            ...
            "--print" => {
                // Kronon Code compat: --print makes output non-interactive
...
fn bare_slash_command_guidance(command_name: &str) -> Option<String> {
    ...
    let guidance = if slash_command.resume_supported {
        format!(
            "`kronon {command_name}` is a slash command. Use `kronon --resume SESSION.jsonl /{command_name}` or start `kronon` and run `/{command_name}`."
        )
    } else {
        format!(
            "`kronon {command_name}` is a slash command. Start `kronon` and run `/{command_name}` inside the REPL."
        )
    };
    Some(guidance)
}
...
fn parse_direct_slash_cli_action(
    ...
    match SlashCommand::parse(&raw) {
        ...
        Ok(Some(command)) => Err({
            let _ = command;
            format!(
                "slash command {command_name} is interactive-only. Start `kronon` and run it there, or use `kronon --resume SESSION.jsonl {command_name}` / `kronon --resume {latest} {command_name}` when the command is marked [resume] in /help.",
                command_name = rest[0],
...
fn format_unknown_direct_slash_command(name: &str) -> String {
    ...
    message.push_str("\nRun `kronon --help` for CLI usage, or start `kronon` and use /help.");
    message
}
...
fn omc_compatibility_note_for_unknown_slash_command(name: &str) -> Option<&'static str> {
    name.starts_with("oh-my-claudecode:")
        .then_some(
            "Compatibility note: `/oh-my-claudecode:*` is a Claude Code/OMC plugin command. `kronon` does not yet load plugin slash commands, Claude statusline stdin, or OMC session hooks.",
        )
}
...
/// Starts a minimal Model Context Protocol server that exposes kronon's
...
fn run_worker_state(output_format: CliOutputFormat) -> Result<(), Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    let state_path = cwd.join(".kronon").join("worker-state.json");
...
fn check_auth_health() -> DiagnosticCheck {
    ...
    match load_oauth_credentials() {
        Ok(Some(token_set)) => {
            ...
            if expired {
                details.push(
                    "Suggested action  kronon login to refresh local OAuth credentials".to_string(),
                );
            }
            DiagnosticCheck::new(
                "Auth",
...
fn render_resume_usage() -> String {
  ...
  Auto-save        .kronon/sessions/<session-id>.{PRIMARY_SESSION_EXTENSION}
    ...
    match command {
        ...
        SlashCommand::Skills { args } => {
            if let SkillSlashDispatch::Invoke(_) = classify_skills_slash_command(args.as_deref()) {
                return Err(
                    "resumed /skills invocations are interactive-only; start `kronon` and run `/skills <skill>` in the REPL".into(),
                );
            }
            let cwd = env::current_dir()?;
            Ok(ResumeCommandOutcome {
...
/// commit (from `--base-commit` flag or `.kronon-base` file). Emits a warning to
...
\x1b[38;5;202m╚═╝  ╚═╝╚═╝  ╚═╝ ╚═════╝ ╚═╝  ╚═══╝ ╚═════╝ ╚═╝  ╚═══╝\x1b[0m \x1b[38;5;208mCode\x1b[0m ⏳\n\n\
  ...
  Type \x1b[1m/help\x1b[0m for commands · \x1b[1m/status\x1b[0m for live context · \x1b[2m/resume latest\x1b[0m jumps back to the newest session · \x1b[1m/diff\x1b[0m then \x1b[1m/commit\x1b[0m to ship · \x1b[2mTab\x1b[0m for workflow completions · \x1b[2mShift+Enter\x1b[0m for newline",
    ...
    fn print_mcp(
        ...
        // `kronon mcp serve` starts a stdio MCP server exposing kronon's built-in
...
fn resolve_managed_session_path(session_id: &str) -> Result<PathBuf, Box<dyn std::error::Error>> {
    ...
    // `.kronon/sessions/<id>.{jsonl,json}` without the per-workspace hash
    // subdirectory. Walk up from `directory` to the `.kronon/sessions/` root
...
fn list_managed_sessions() -> Result<Vec<ManagedSessionSummary>, Box<dyn std::error::Error>> {
    ...
    // flat `.kronon/sessions/` root so users do not lose access to existing
...
fn format_missing_session_reference(reference: &str) -> String {
    format!(
        "session not found: {reference}\nHint: managed sessions live in .kronon/sessions/. Try `{LATEST_SESSION_REFERENCE}` for the most recent session or `/session list` in the REPL."
    )
}

fn format_no_managed_sessions() -> String {
    format!(
        "no managed sessions found in .kronon/sessions/\nStart `kronon` to create a session, then rerun with `--resume {LATEST_SESSION_REFERENCE}`."
    )
}
...
fn render_help_topic(topic: LocalHelpTopic) -> String {
  ...
  Related          /status · kronon --resume latest /status"
            .to_string(),
        LocalHelpTopic::Sandbox => "Sandbox
  Usage            kronon sandbox
  Purpose          inspect the resolved sandbox and isolation state for the current directory
  Output           namespace, network, filesystem, and fallback details
  Related          /sandbox · kronon status"
            .to_string(),
        LocalHelpTopic::Doctor => "Doctor
  Usage            kronon doctor
  Purpose          diagnose local auth, config, workspace, sandbox, and build metadata
  Output           local-only health report; no provider request or session resume required
  Related          /doctor · kronon --resume latest /doctor"
            .to_string(),
    }
}
...
impl AnthropicRuntimeClient {
    fn new(
        ...
        // kronon at its fake Anthropic endpoint. We also attach a
...
fn format_context_window_blocked_error(session_id: &str, error: &api::ApiError) -> String {
    ...
    lines.push(format!(
        "  Resume compact   kronon --resume {session_id} /compact"
    ));
...
fn print_help_to(out: &mut impl Write) -> io::Result<()> {
    writeln!(out, "kronon v{VERSION}")?;
    writeln!(out)?;
    writeln!(out, "Usage:")?;
    writeln!(
        out,
        "  kronon [--model MODEL] [--allowedTools TOOL[,TOOL...]]"
    )?;
    writeln!(out, "      Start the interactive REPL")?;
    writeln!(
        out,
        "  kronon [--model MODEL] [--output-format text|json] prompt TEXT"
    )?;
    writeln!(out, "      Send one prompt and exit")?;
    writeln!(
        out,
        "  kronon [--model MODEL] [--output-format text|json] TEXT"
    )?;
    writeln!(out, "      Shorthand non-interactive prompt mode")?;
    writeln!(
        out,
        "  kronon --resume [SESSION.jsonl|session-id|latest] [/status] [/compact] [...]"
    )?;
    writeln!(
        out,
        "      Inspect or maintain a saved session without entering the REPL"
    )?;
    writeln!(out, "  kronon help")?;
    writeln!(out, "      Alias for --help")?;
    writeln!(out, "  kronon version")?;
    writeln!(out, "      Alias for --version")?;
    writeln!(out, "  kronon status")?;
    writeln!(
        out,
        "      Show the current local workspace status snapshot"
    )?;
    writeln!(out, "  kronon sandbox")?;
    writeln!(out, "      Show the current sandbox isolation snapshot")?;
    writeln!(out, "  kronon doctor")?;
    writeln!(
        out,
        "      Diagnose local auth, config, workspace, and sandbox health"
    )?;
    writeln!(out, "  kronon dump-manifests")?;
    writeln!(out, "  kronon bootstrap-plan")?;
    writeln!(out, "  kronon agents")?;
    writeln!(out, "  kronon mcp")?;
    writeln!(out, "  kronon skills")?;
    writeln!(out, "  kronon system-prompt [--cwd PATH] [--date YYYY-MM-DD]")?;
    writeln!(out, "  kronon login")?;
    writeln!(out, "  kronon logout")?;
    writeln!(out, "  kronon init")?;
    writeln!(
        out,
        "  kronon export [PATH] [--session SESSION] [--output PATH]"
    )?;
    writeln!(
        out,
    ...
    writeln!(
        out,
        "  REPL turns auto-save to .kronon/sessions/<session-id>.{PRIMARY_SESSION_EXTENSION}"
    )?;
    writeln!(
        out,
    ...
    writeln!(out, "  kronon --model claude-opus \"summarize this repo\"")?;
    writeln!(
        out,
        "  kronon --output-format json prompt \"explain src/main.rs\""
    )?;
    writeln!(out, "  kronon --compact \"summarize Cargo.toml\" | wc -l")?;
    writeln!(
        out,
        "  kronon --allowedTools read,glob \"summarize Cargo.toml\""
    )?;
    writeln!(out, "  kronon --resume {LATEST_SESSION_REFERENCE}")?;
    writeln!(
        out,
        "  kronon --resume {LATEST_SESSION_REFERENCE} /status /diff /export notes.txt"
    )?;
    writeln!(out, "  kronon agents")?;
    writeln!(out, "  kronon mcp show my-server")?;
    writeln!(out, "  kronon /skills")?;
    writeln!(out, "  kronon doctor")?;
    writeln!(out, "  kronon login")?;
    writeln!(out, "  kronon init")?;
    writeln!(out, "  kronon export")?;
    writeln!(out, "  kronon export conversation.md")?;
    Ok(())
}
...
mod tests {
    ...
    fn context_window_preflight_errors_render_recovery_steps() {
        ...
        assert!(
            rendered.contains("Resume compact   kronon --resume session-issue-32 /compact"),
            "{rendered}"
        );
        assert!(
    ...
    fn default_permission_mode_uses_project_config_when_env_is_unset() {
        ...
        std::fs::create_dir_all(cwd.join(".kronon")).expect("project config dir should exist");
        std::fs::create_dir_all(&config_home).expect("config home should exist");
        std::fs::write(
            cwd.join(".kronon").join("settings.json"),
        ...
        std::env::set_var("KRONON_CONFIG_HOME", &config_home);
        std::env::remove_var("RUSTY_CLAUDE_PERMISSION_MODE");
    ...
    fn parses_direct_agents_mcp_and_skills_slash_commands() {
        ...
        assert!(error.contains("kronon --resume SESSION.jsonl /status"));
    }
    ...
    fn rejects_unknown_options_with_helpful_guidance() {
        ...
        assert!(error.contains("kronon --help"));
    }
    ...
    fn repl_help_includes_shared_commands_and_exit() {
        ...
        assert!(help.contains("Auto-save            .kronon/sessions/<session-id>.jsonl"));
    ...
    fn init_help_mentions_direct_subcommand() {
        ...
        assert!(help.contains("kronon help"));
        assert!(help.contains("kronon version"));
        assert!(help.contains("kronon status"));
        assert!(help.contains("kronon sandbox"));
        assert!(help.contains("kronon init"));
        assert!(help.contains("kronon agents"));
        assert!(help.contains("kronon mcp"));
        assert!(help.contains("kronon skills"));
        assert!(help.contains("kronon /skills"));
    }
...
UU conflicted.rs",
    ...
    fn help_mentions_jsonl_resume_examples() {
        ...
        print_help_to(&mut help).expect("help should render");
        let help = String::from_utf8(help).expect("help should be utf8");
        assert!(help.contains("kronon --resume [SESSION.jsonl|session-id|latest]"));
        assert!(help.contains("Use `latest` with --resume, /resume, or /session switch"));
        assert!(help.contains("kronon --resume latest"));
        assert!(help.contains("kronon --resume latest /status /diff /export notes.txt"));
    }
    ...
    fn managed_sessions_default_to_jsonl_and_resolve_legacy_json() {
        ...
        let legacy_path = workspace.join(".kronon/sessions/legacy.json");
    ...
    fn resume_usage_mentions_latest_shortcut() {
        ...
        assert!(usage.contains(".kronon/sessions/<session-id>.jsonl"));
    ...
    fn temp_workspace(label: &str) -> PathBuf {
        let nanos = std::time::SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("kronon-cli-{label}-{nanos}"))
    }
