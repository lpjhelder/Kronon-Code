...
fn upstream_repo_candidates(primary_repo_root: &Path) -> Vec<PathBuf> {
    ...
    for ancestor in primary_repo_root.ancestors().take(4) {
        candidates.push(ancestor.join("kronon-code"));
        candidates.push(ancestor.join("kronond-code"));
    }

    candidates.push(primary_repo_root.join("reference-source").join("kronon-code"));
    candidates.push(primary_repo_root.join("vendor").join("kronon-code"));
