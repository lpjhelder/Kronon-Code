...
pub const KRONON_SETTINGS_SCHEMA_NAME: &str = "SettingsSchema";
...
/// OAuth client configuration used by the main Kronon runtime.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl ConfigLoader {
    ...
    pub fn discover(&self) -> Vec<ConfigEntry> {
        let user_legacy_path = self.config_home.parent().map_or_else(
            || PathBuf::from(".kronon.json"),
            |parent| parent.join(".kronon.json"),
        );
        vec![
            ConfigEntry {
            ...
            ConfigEntry {
                ...
                path: self.cwd.join(".kronon.json"),
            },
            ConfigEntry {
                source: ConfigSource::Project,
                path: self.cwd.join(".kronon").join("settings.json"),
            },
            ConfigEntry {
                source: ConfigSource::Local,
                path: self.cwd.join(".kronon").join("settings.local.json"),
            },
        ]
    }
...
pub fn default_config_home() -> PathBuf {
    std::env::var_os("KRONON_CONFIG_HOME")
        .map(PathBuf::from)
        .or_else(|| std::env::var_os("HOME").map(|home| PathBuf::from(home).join(".kronon")))
        .unwrap_or_else(|| PathBuf::from(".kronon"))
}
...
fn read_optional_json_object(path: &Path) -> Result<Option<ParsedConfigFile>, ConfigError> {
    let is_legacy_config = path.file_name().and_then(|name| name.to_str()) == Some(".kronon.json");
...
mod tests {
    use super::{
        ...
        RuntimePluginConfig, KRONON_SETTINGS_SCHEMA_NAME,
    };
    use crate::json::JsonValue;
    ...
    fn rejects_non_object_settings_files() {
        ...
        let home = root.join("home").join(".kronon");
    ...
    fn loads_and_merges_claude_code_config_files_by_precedence() {
        ...
        let cwd = root.join("project");
        let home = root.join("home").join(".kronon");
        fs::create_dir_all(cwd.join(".kronon")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");

        fs::write(
            home.parent().expect("home parent").join(".kronon.json"),
        ...
        fs::write(
            cwd.join(".kronon").join("settings.local.json"),
        ...
        assert_eq!(KRONON_SETTINGS_SCHEMA_NAME, "SettingsSchema");
    ...
    fn rejects_invalid_hook_entries_before_merge() {
        ...
        let cwd = root.join("project");
        let home = root.join("home").join(".kronon");
        let project_settings = cwd.join(".kronon").join("settings.json");
        fs::create_dir_all(cwd.join(".kronon")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");
