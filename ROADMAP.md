...
Turn Kronon-Code into the most **automatable** coding harness:
...
This roadmap assumes the primary users are **kronons wired through hooks, plugins, sessions, and channel events**.

## Definition of "automatable"

A automatable harness is:
...
- tmux state
- kronon-hip event stream
- git/worktree state
- test state
- gateway/plugin/MCP runtime state

### 3. Events are too log-shaped
- kronons currently infer too much from noisy text
...
### 7. Human UX still leaks into kronon workflows
...
Acceptance:
- a kronon can operate a coding worker without raw send-keys as the primary control plane

## Phase 2 — Event-Native Kronon-hip Integration
...
Acceptance:
- kronon-hip consumes typed lane events
...
- kronons stop inferring state from raw build spam
...
## Phase 4 — Kronons-First Task Execution
...
Acceptance:
- kronons can dispatch work without relying on long natural-language prompt blobs alone
...
### 12. Kronon-native dashboards / lane board
...
Acceptance:
- kronons can query status directly
...
5. Surface `doctor` / preflight diagnostics in onboarding docs and help — **done**: README + USAGE now put `kronon doctor` / `/doctor` in the first-run path and point at the built-in preflight report
...
8. Promote `doctor` from slash-only to top-level CLI entrypoint — **done**: `kronon doctor` is now a local shell entrypoint with regression coverage for direct help and health-report output
9. Make machine-readable status commands actually machine-readable — **done**: `kronon --output-format json status` and `kronon --output-format json sandbox` now emit structured JSON snapshots instead of prose tables
10. Unify legacy config/skill namespaces in user-facing output — **done**: skills/help JSON/text output now present `.kronon` as the canonical namespace and collapse legacy roots behind `.kronon`-shaped source ids/labels
...
7. Canonical lane event schema in kronon-hip — **done**: `LaneEvent` enum with `Started/Blocked/Failed/Finished` variants, `LaneEvent::new()` typed constructor, `tools/src/lib.rs` integration
...
19. **Subcommand help falls through into runtime/API path** — **done**: `kronon doctor --help`, `kronon status --help`, `kronon sandbox --help`, and nested `mcp`/`skills` help are now intercepted locally without runtime/provider startup, with regression tests covering the direct CLI paths.
...
41. **Phantom completions root cause: global session store has no per-worktree isolation** —
    ...
    **Proposed fix.** Bind every session to its workspace root + branch at creation time and refuse to drive it from any other CWD.
    ...
    - Surface a `branch.workspace_mismatch` lane event so kronon-hip stops counting wrong-CWD writes as lane completions.
...
### Session B — kronon-hip lane events
Focus:
...
We should consider Kronon-Code materially more automatable when:
- a kronon can start a worker and know with certainty when it is ready
- kronons no longer accidentally type tasks into the shell
- stale-branch failures are identified before they waste debugging time
- kronon-hip reports machine states, not just tmux prose
...
Kronon-Code should evolve from:
- a CLI a human can also drive

to:
- a **kronon-native execution runtime**
...
`worker_boot.rs` has a solid `WorkerStatus` state machine (`Spawning → TrustRequired → ReadyForPrompt → Running → Finished/Failed`). It is exported from `runtime/src/lib.rs` as a public API. But Kronon-Code is a **plugin** loaded inside the `opencode` binary — it cannot add HTTP routes to `opencode serve`. The HTTP server is 100% owned by the upstream opencode process (v1.3.15).
...
3. Writing `WorkerStatus` to a well-known file path (`.kronon/worker-state.json`) that an external observer can poll.

**Recommended path:** Option 3 — emit `WorkerStatus` transitions to `.kronon/worker-state.json` on every state change. This is purely within Kronon-Code's plugin scope, requires no upstream changes, and gives kronon-hip a file it can poll to distinguish a truly stalled worker from a quiet-but-progressing one.

**Action item:** Wire `WorkerRegistry::transition()` to atomically write `.kronon/worker-state.json` on every state transition. Add a `kronon state` CLI subcommand that reads and prints this file. Add regression test.
...
`WorkerCreate` accepts a `trusted_roots: Vec<String>` parameter. If the caller omits it (or passes `[]`), every new worker immediately enters `TrustRequired` and stalls — requiring manual intervention to advance to `ReadyForPrompt`. There is no mechanism to configure a default allowlist in `settings.json` or `.kronon/settings.json`.

**Impact:** Batch tooling (kronon-hip, lane orchestrators) must pass `trusted_roots` explicitly on every `WorkerCreate` call. If a batch script forgets the field, all workers in that batch stall silently at `trust_required`. This was the root cause of several "batch 8 lanes not advancing" incidents.
...
**Decision:** `kronon state` reading `.kronon/worker-state.json` is the **blessed observability contract** for kronon-hip and downstream tooling. This is not a stepping-stone — it is the supported surface. Build against it.

**Rationale:**
- Kronon-Code is a plugin running inside the opencode binary. It cannot add HTTP routes to `opencode serve` — that server belongs to upstream sst/opencode.
...
- `kronon state --output-format json` gives kronon-hip everything it needs: `status`, `is_ready`, `seconds_since_update`, `trust_gate_cleared`, `last_event`, `updated_at`.
...
**What downstream tooling (kronon-hip) should do:**
1. After `WorkerCreate`, poll `.kronon/worker-state.json` (or run `kronon state --output-format json`) in the worker's CWD at whatever interval makes sense (e.g. 5s).
