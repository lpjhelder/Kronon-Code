...
Turn Kronon-Code into the most **clawable** coding harness:
...
We should consider Kronon-Code materially more clawable when:
...
**Recommended path:** Option 3 — emit `WorkerStatus` transitions to `.claw/worker-state.json` on every state change. This is purely within Kronon-Code's plugin scope, requires no upstream changes, and gives clawhip a file it can poll to distinguish a truly stalled worker from a quiet-but-progressing one.
